...
`ingest-bls-observations` requires the canonical UUIDs of previously persisted CPI and Employment Situation events and an explicit limit from 1 through 100. It binds those events to the official CPI-U all-items and total nonfarm payroll series in that order, retrieves bounded raw latest and previous values, and idempotently persists complete source-cited UTC snapshots with the fixed BLS observation ingestion audit identity. Successful cycles report the complete processed count only after persistence finishes; retrieval, persistence, configuration, and cancellation failures return contextual errors with the partial processed count and emit no success output. The command uses neither OpenAI nor semantic retrieval. Scheduling, calculated changes, consensus enrichment, revision history, surprise calculations, generated analysis, HTTP delivery, and UI presentation remain deferred.
