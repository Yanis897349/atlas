...
func TestRepositoryValidatesSimilarityQueryBeforePostgreSQL(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "underflowing norm", provider: "provider", model: "model", queryVector: []float32{1e-30}, limit: 1, contains: "query vector must have finite non-zero cosine norm"},
		{name: "overflowing norm", provider: "provider", model: "model", queryVector: []float32{1e30}, limit: 1, contains: "query vector must have finite non-zero cosine norm"},
