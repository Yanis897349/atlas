...
func TestEmbedSourceRecordsRejectsInvalidProviderBatches(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "zero norm", batch: withBatch(valid, func(batch *EmbeddingBatch) { batch.Embeddings[0].Vector = []float32{0, 0} }), contains: "embedding 0 vector must have non-zero norm"},
