...
func TestAdapterFetchEventsReportsRetrievalFailures(t *testing.T) {
	...
	tests := []struct {
		name   string
		client ecb.HTTPClient
		want   string
	}{
		{name: "transport", client: &recordingClient{err: errors.New("connection reset")}, want: "fetch ECB calendar: connection reset"},
		...
		{name: "read", client: &recordingClient{response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(failingReader{})}}, want: "read ECB calendar"},
		{
			name: "oversized",
