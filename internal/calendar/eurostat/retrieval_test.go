...
func TestAdapterFetchEventsReportsRetrievalFailures(t *testing.T) {
	...
	tests := []struct {
		name   string
		client eurostat.HTTPClient
		want   string
	}{
		{name: "transport", client: &recordingClient{err: errors.New("connection reset")}, want: "fetch Eurostat calendar: connection reset"},
		...
		{name: "read", client: &recordingClient{response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(failingReader{})}}, want: "read Eurostat calendar"},
		{
			name: "oversized",
