...
func TestRunValidatesCommandBeforeConfiguration(t *testing.T) {
	tests := [][]string{nil, {"unknown"}, {"migrate", "extra"}, {"ingest-bea", "extra"}, {"ingest-census", "extra"}, {"ingest-eurostat", "extra"}}
...
func TestRunRecognizesCommandsBeforeRequiringApplicationDatabaseURL(t *testing.T) {
	for _, command := range []string{"migrate", "ingest-rss", "ingest-bls", "ingest-fed", "ingest-ecb", "ingest-bea", "ingest-census", "ingest-eurostat"} {
